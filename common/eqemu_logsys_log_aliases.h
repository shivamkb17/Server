...
#define LogCharacterSets(message, ...) do {\
    if (logsys->IsLogEnabled(Logs::General, Logs::CharacterSets))\
        OutF(logsys, Logs::General, Logs::CharacterSets, __FILE__, __func__, __LINE__, message, ##__VA_ARGS__);\
} while (0)

#define LogCharacterSetsDetail(message, ...) do {\
    if (logsys->IsLogEnabled(Logs::Detail, Logs::CharacterSets))\
        OutF(logsys, Logs::Detail, Logs::CharacterSets, __FILE__, __func__, __LINE__, message, ##__VA_ARGS__);\
} while (0)
